...
const exerciseName = window.document.querySelector('input[placeholder="Ej: Press inclinado smith"]');
const setter = Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype, 'value').set;
setter.call(exerciseName, 'Remo de prueba');
exerciseName.dispatchEvent(new window.Event('input', { bubbles: true }));
button('GUARDAR EJERCICIO').click();
await waitFor(() => window.document.body.textContent.includes('REMO DE PRUEBA'));
assert.equal((await storage.get('custom-exercises'))[0].name, 'Remo de prueba');
...
await waitFor(() => !!window.document.querySelector('button[aria-label="Marcar completada"]'));
